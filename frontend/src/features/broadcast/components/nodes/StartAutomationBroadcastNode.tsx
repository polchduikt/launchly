...
export const StartAutomationBroadcastNode: React.FC<StartAutomationNodeProps> = ({ selected, data = {} }) => {
  const name = data?.automationName || '';

  return (
    <div
      className={`w-60 bg-white border rounded-2xl p-4 shadow-xs transition-all select-none ${
        selected ? 'border-emerald-500 ring-2 ring-emerald-50' : 'border-slate-200'
      }`}
    >
      <Handle
        type="target"
