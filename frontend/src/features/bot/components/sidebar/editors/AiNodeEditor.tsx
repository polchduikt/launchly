...
export const AiNodeEditor: React.FC<AiNodeEditorProps> = ({ data, handleChange, editorState }) => {
  ...
  useEffect(() => {
    const keys = ['launchly_key_chatgpt', 'launchly_key_claude', 'launchly_key_deepseek', 'launchly_key_gemini'];
    ...
    const list = [];
    if (localStorage.getItem('launchly_key_chatgpt')) {
      list.push({
        id: 'chatgpt',
        name: 'ChatGPT',
        icon: (
    ...
    if (localStorage.getItem('launchly_key_claude')) {
      list.push({
        id: 'claude',
        name: 'Claude',
    ...
    if (localStorage.getItem('launchly_key_deepseek')) {
      list.push({
        id: 'deepseek',
        name: 'DeepSeek',
        icon: (
    ...
    if (localStorage.getItem('launchly_key_gemini')) {
      list.push({
        id: 'gemini',
        name: 'Gemini',
        icon: <SiGooglegemini className="w-3.5 h-3.5 text-[#00a6ff]" />
      });
    }
