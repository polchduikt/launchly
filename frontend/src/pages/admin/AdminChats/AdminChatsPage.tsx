      ...
      <div className="flex h-full w-full overflow-hidden bg-canvas text-ink font-['JetBrains_Mono',monospace]">
        ...
        {isFilterCollapsed ? (
          <aside
            ...
            className="w-14 bg-canvas hover:bg-white border-r-2 border-ink h-full flex items-center justify-center shrink-0 cursor-pointer transition select-none z-20"
            title="Показати фільтри"
          >
            <PanelLeftOpen size={22} className="text-ink" />
          </aside>
        ) : (
          <aside className="w-64 bg-canvas border-r-2 border-ink h-full flex flex-col shrink-0 z-20">
            <div className="h-16 px-4 border-b-2 border-ink flex items-center justify-between shrink-0">
              <div className="font-['Anybody',sans-serif] text-xs font-black uppercase text-ink flex items-center gap-1.5">
              ...
              <div className="flex items-center space-x-1">
                {(activeTab !== 'all' || selectedPeriod !== 'all' || searchQuery.trim() || sortOrder !== 'desc') && (
                  <button
                    ...
                    className="p-1 text-ink hover:opacity-70 transition cursor-pointer"
                ...
                <button
                  ...
                  className="p-1 rounded-lg text-ink hover:bg-white border-2 border-transparent hover:border-ink transition cursor-pointer"
            ...
            <div className="space-y-1 shrink-0">
              ...
              <div className="relative w-full">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-ink" size={13} />
                <input
                  type="text"
                  placeholder={t('admin.search_chats_placeholder')}
                  value={searchQuery}
                  onChange={(e) => setSearchQuery(e.target.value)}
                  className="w-full pl-8 pr-3 py-1.5 bg-white border-2 border-ink rounded-xl text-xs font-bold text-ink placeholder-slate-500 focus:outline-none transition shadow-brutal-sm"
                />
              </div>
            </div>
                ...
                {(
                  ...
                  return (
                    <button
                      ...
                      className={`w-full flex items-center justify-between px-3 py-1.5 rounded-xl text-xs font-black uppercase transition cursor-pointer border-2 border-ink ${
                        isActive
                          ? 'bg-ink text-canvas shadow-brutal-sm'
                          : 'bg-white text-ink hover:bg-amber-50'
                      }`}
                    >
                      <div className="flex items-center space-x-2">
                        <IconComponent size={14} className={isActive ? 'text-canvas' : 'text-ink'} />
                        <span>{tab.label}</span>
                      </div>
                    </button>
                  );
        ...
        <aside className="w-80 lg:w-84 bg-canvas border-r-2 border-ink h-full flex flex-col shrink-0 z-10">
          <div className="h-16 px-4 border-b-2 border-ink flex items-center justify-between shrink-0">
            <h3 className="font-['Anybody',sans-serif] text-xs font-black uppercase text-ink flex items-center gap-2">
              <MessageSquare size={16} className="text-ink" />
              <span>{t('admin.chats_title')}</span>
            </h3>
            <span className="px-2.5 py-0.5 rounded-lg bg-white border-2 border-ink text-ink font-black text-[11px] shadow-brutal-sm">
              {tickets.length}
            </span>
          </div>

          <div className="flex-1 overflow-y-auto custom-scrollbar divide-y-2 divide-ink/20">
            {isTicketsLoading ? (
              <div className="flex items-center justify-center py-16 text-ink text-xs font-bold">
                ...
                return (
                  <div
                    ...
                    className={`p-3.5 flex items-start gap-3 transition cursor-pointer relative ${
                      isSelected ? 'bg-white border-l-4 border-ink' : 'hover:bg-white/60'
                    }`}
                  >
                    <div className="relative shrink-0">
                      {c.userAvatar ? (
                        <img src={c.userAvatar} alt={c.userName} className="w-10 h-10 rounded-xl object-cover border-2 border-ink" />
                      ) : (
                        <div className="w-10 h-10 rounded-xl bg-amber-200 border-2 border-ink flex items-center justify-center font-black text-ink text-xs shadow-brutal-sm">
                          {(c.userName || 'U')[0].toUpperCase()}
                        </div>
                      )}
                      {c.unread && (
                        <span className="absolute -top-1 -right-1 w-3.5 h-3.5 rounded-full bg-rose-500 border-2 border-ink" />
                      )}
                    </div>

                    <div className="flex-1 min-w-0 space-y-1">
                      <div className="flex items-center justify-between">
                        <span className={`text-xs truncate ${c.unread ? 'font-black text-ink' : 'font-bold text-ink'}`}>
                          {c.userName}
                        </span>
                      ...
                      <p className={`text-[11px] truncate ${c.unread ? 'font-black text-ink' : 'text-slate-700 font-bold'}`}>
                        {c.lastMessage || '...'}
                      </p>
                    </div>
                    ...
                    <button
                      ...
                      className={`p-1 rounded-lg transition hover:bg-canvas shrink-0 ${
        ...
        {selectedTicket ? (
          <div className="flex-1 flex flex-col h-full bg-canvas min-w-0">
            ...
            <header className="h-16 px-6 bg-canvas border-b-2 border-ink flex items-center justify-between shrink-0">
              <div className="flex items-center space-x-3.5 min-w-0">
                {selectedTicket.userAvatar ? (
                  <img src={selectedTicket.userAvatar} alt={selectedTicket.userName} className="w-10 h-10 rounded-xl object-cover border-2 border-ink shrink-0" />
                ) : (
                  <div className="w-10 h-10 rounded-xl bg-amber-200 border-2 border-ink flex items-center justify-center font-black text-ink text-sm shrink-0 shadow-brutal-sm">
                ...
                <div className="space-y-0.5 min-w-0">
                  <div className="flex items-center space-x-2">
                    <h3 className="font-['Anybody',sans-serif] text-sm font-black uppercase text-ink truncate">{selectedTicket.userName}</h3>
                    <span className="px-2 py-0.5 rounded-lg bg-white border-2 border-ink text-ink font-black text-[10px] uppercase font-mono">
              ...
              {(selectedTicket.messages || []).map(msg => {
                ...
                return (
                  ...
                  >
                    ...
                    <div
                      className={`p-3.5 rounded-2xl text-xs leading-relaxed border-2 border-ink shadow-brutal-sm break-all [word-break:break-word] whitespace-pre-wrap ${
                        isManagerMsg
                          ? 'bg-ink text-canvas rounded-tr-xs font-bold'
                          : 'bg-white text-ink rounded-tl-xs font-bold'
                      }`}
                    >
                      {msg.text}
                    </div>
            ...
            <div className="p-4 bg-canvas border-t-2 border-ink space-y-3 shrink-0">
              {(selectedTicket.status as string) === 'RESOLVED' || (selectedTicket.status as string) === 'CLOSED' ? (
                <div className="p-3 bg-white border-2 border-ink rounded-2xl text-center space-y-1 shadow-brutal-sm">
                  <div className="text-xs font-black uppercase text-ink flex items-center justify-center gap-1.5">
                ...
                <div className="p-4 bg-white border-2 border-ink rounded-2xl flex flex-col items-center justify-center space-y-2 text-center shadow-brutal-sm">
                  ...
                  <button
                    ...
                    className="px-6 py-2.5 bg-ink hover:bg-[#2A2A2A] text-canvas font-black uppercase border-2 border-ink text-xs rounded-xl transition flex items-center space-x-2 shadow-brutal-sm cursor-pointer disabled:opacity-50"
                  >
                ...
                <div className="p-3 bg-white border-2 border-ink rounded-2xl flex items-center justify-between gap-3 shadow-brutal-sm">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className="px-3.5 py-1.5 bg-rose-200 hover:bg-rose-300 text-rose-950 border-2 border-ink rounded-xl text-xs font-black uppercase transition flex items-center space-x-1.5 cursor-pointer shadow-brutal-sm disabled:opacity-50"
                    >
                      <X size={14} />
              ...
              <form onSubmit={handleSendMessage} className="flex items-center space-x-3">
                <input
                  ...
                  className="flex-1 px-4 py-2.5 bg-white border-2 border-ink rounded-2xl text-xs font-bold text-ink placeholder-slate-500 focus:outline-none transition shadow-brutal-sm disabled:opacity-50 disabled:cursor-not-allowed"
                />
                <button
                  type="submit"
                  disabled={
                  ...
                  className="px-5 py-2.5 bg-ink hover:bg-[#2A2A2A] text-canvas font-black uppercase text-xs border-2 border-ink rounded-2xl disabled:opacity-40 disabled:cursor-not-allowed transition flex items-center space-x-1.5 shadow-brutal-sm cursor-pointer"
                >
        ...
        {selectedTicket && (
          isProfileCollapsed ? (
            <aside
              ...
              className="w-14 bg-canvas hover:bg-white border-l-2 border-ink h-full flex items-center justify-center shrink-0 cursor-pointer transition select-none z-10"
              title="Показати інфо про клієнта"
            >
              <PanelRightOpen size={22} className="text-ink" />
            </aside>
          ) : (
            <aside className="w-80 bg-canvas border-l-2 border-ink h-full flex flex-col shrink-0 z-10 select-text">
              <div className="h-16 px-5 border-b-2 border-ink flex items-center justify-between shrink-0">
                <h5 className="font-['Anybody',sans-serif] text-xs font-black uppercase text-ink">
                  {t('admin.client_info')}
                </h5>
                <button
              ...
              <div className="text-center space-y-2 border-b-2 border-ink pb-4">
                {selectedTicket.userAvatar ? (
                  <img src={selectedTicket.userAvatar} alt={selectedTicket.userName} className="w-16 h-16 rounded-2xl object-cover border-2 border-ink mx-auto shadow-brutal-sm" />
                ) : (
                  <div className="w-16 h-16 rounded-2xl bg-amber-200 border-2 border-ink flex items-center justify-center font-black text-ink text-xl mx-auto shadow-brutal-sm">
                    {(selectedTicket.userName || 'U')[0].toUpperCase()}
                  </div>
                )}
                <div>
                  <h4 className="font-black text-ink text-sm">{selectedTicket.userName}</h4>
                  <p className="text-xs text-slate-700 font-mono font-bold truncate">{selectedTicket.userEmail}</p>
                </div>
                <div className="flex items-center justify-center gap-2 pt-1">
                  <span className="px-2.5 py-0.5 rounded-lg bg-white border-2 border-ink text-ink font-black text-[10px] uppercase font-mono">
                    {selectedTicket.userPlan}
                  </span>
                  <span className={`px-2.5 py-0.5 rounded-lg border-2 border-ink font-black text-[10px] uppercase font-mono ${
              ...
              <div className="space-y-3 text-xs">
                <h5 className="font-black text-[10px] uppercase tracking-wider text-ink">
                  {t('admin.activity_and_stats')}
                </h5>
                
                <div className="bg-white p-3.5 rounded-2xl border-2 border-ink space-y-2.5 font-bold shadow-brutal-sm">
                  <div className="flex items-center justify-between text-ink">
                    <span className="flex items-center gap-1.5"><Calendar size={13} /> {t('admin.reg_date')}</span>
                    <span className="font-black text-ink font-mono text-[11px]">
              ...
              <div className="space-y-3 text-xs">
                ...
                <div className="bg-white p-3.5 rounded-2xl border-2 border-ink space-y-2 font-bold shadow-brutal-sm">
                  <div className="flex items-center justify-between text-ink">
                    <span className="flex items-center gap-1.5"><Bot size={13} /> {t('admin.bots_label')}</span>
                    <span className="font-black font-mono text-ink">{selectedTicket.botsCount}</span>
                  </div>
                  <div className="flex items-center justify-between text-ink">
                    <span className="flex items-center gap-1.5"><Workflow size={13} /> {t('admin.automations_label')}</span>
                    <span className="font-black font-mono text-ink">{selectedTicket.automationsCount}</span>
                  </div>
                  <div className="flex items-center justify-between text-ink">
                    <span className="flex items-center gap-1.5"><Send size={13} /> {t('admin.broadcasts_label')}</span>
                    <span className="font-black font-mono text-ink">{selectedTicket.broadcastsCount}</span>
                  </div>
                  <div className="flex items-center justify-between text-ink">
                    <span className="flex items-center gap-1.5"><Users size={13} /> {t('admin.subscribers')}</span>
                    <span className="font-black font-mono text-ink">{selectedTicket.contactsCount ?? 0}</span>
                  </div>
                  <div className="flex items-center justify-between text-ink">
                    <span className="flex items-center gap-1.5"><MessageSquare size={13} /> {t('admin.messages_sent')}</span>
                    <span className="font-black font-mono text-ink">{selectedTicket.messagesCount ?? 0}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="pt-2">
                <button
                  ...
                  className="w-full py-2.5 px-3 bg-ink hover:bg-[#2A2A2A] text-canvas rounded-xl border-2 border-ink text-xs font-black uppercase transition flex items-center justify-center cursor-pointer shadow-brutal-sm"
                >
        ...
        {showDetailModal && selectedTicket && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/40 backdrop-blur-xs p-4 animate-in fade-in duration-150 font-['JetBrains_Mono',monospace]">
            <div className="bg-canvas border-4 border-ink rounded-3xl w-full max-w-6xl h-[780px] max-h-[92vh] p-6 sm:p-7 shadow-brutal-2xl flex flex-col justify-between space-y-4 overflow-hidden text-ink">
                ...
                <div className="flex items-center space-x-3.5">
                  {selectedTicket.userAvatar ? (
                    <img
                      ...
                      className="w-12 h-12 rounded-2xl object-cover border-2 border-ink"
                    />
                  ) : (
                    <div className="w-12 h-12 rounded-2xl bg-amber-200 border-2 border-ink flex items-center justify-center font-black text-ink text-lg shadow-brutal-sm">
                  ...
                  <div className="space-y-0.5">
                    <div className="flex items-center space-x-2">
                      <h3 className="font-['Anybody',sans-serif] text-base font-black uppercase text-ink leading-tight">{selectedTicket.userName}</h3>
                      <span className="px-2 py-0.5 rounded-lg bg-white border-2 border-ink text-ink font-black text-[10px] uppercase">
                        {selectedTicket.userRole || 'ROLE_OWNER'}
                      </span>
                      <span className={`px-2 py-0.5 rounded-lg text-[10px] font-black uppercase border-2 border-ink ${
                        selectedTicket.accountActive !== false
                          ? 'bg-emerald-200 text-emerald-950'
                          : 'bg-rose-200 text-rose-950'
                      }`}>
              ...
              <div className="flex items-center justify-between gap-2 bg-white p-2.5 rounded-2xl border-2 border-ink shrink-0 shadow-brutal-sm">
                <div className="flex items-center space-x-1">
                  <span className="text-[11px] font-black uppercase text-ink mr-1 flex items-center gap-1">
                    <Calendar size={13} />
                    {t('admin.period_label')}
                  </span>
                  {(
              ...
              <div className="grid grid-cols-6 gap-2 shrink-0">
                ...
                <div className="bg-white border-2 border-ink rounded-xl p-2.5 shadow-brutal-sm">
                  ...
                  <div className="text-xs font-black uppercase text-ink mt-1 truncate">
                    {selectedTicket.userPlan}
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-3.5 flex-1 min-h-0 overflow-hidden">
                <div className="lg:col-span-5 flex flex-col space-y-3 min-h-0 h-full overflow-hidden">
                  ...
                  <div className="flex-1 flex flex-col min-h-0 border-2 border-ink rounded-2xl bg-white p-3 overflow-hidden shadow-brutal-sm">
                    ...
                    <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 pt-2 pr-1">
                      {isDetailLoading ? (
                        <div className="flex items-center justify-center py-6 text-ink text-xs font-bold">
                          <Loader2 size={16} className="animate-spin mr-2" />
                          ...
                          >
                            <div className="flex items-start justify-between gap-2">
                              <div className="flex items-center space-x-2 min-w-0">
                                <span
                                  className={`w-2.5 h-2.5 rounded-full shrink-0 border border-ink ${
                                    auto.active ? 'bg-emerald-400' : 'bg-slate-300'
                                  }`}
                                />
                                <span className="font-black text-ink text-xs truncate group-hover:underline transition">
                                  {auto.name}
                                </span>
                              </div>
                              <div className="flex items-center space-x-1 shrink-0">
                                <span className="px-1.5 py-0.5 rounded bg-white border border-ink text-[9px] font-black text-ink uppercase font-mono">
                                  {auto.triggerType}
                                </span>
                        ...
                        userDetailData.broadcasts.map((b) => (
                          ...
                          >
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <div className="flex items-center space-x-1 shrink-0">
                                <span
                                  className={`px-1.5 py-0.5 rounded text-[9px] font-black uppercase font-mono border border-ink ${
                                    b.status === 'COMPLETED'
                                      ? 'bg-emerald-200 text-emerald-950'
                                      : b.status === 'RUNNING'
                ...
                <div className="lg:col-span-7 border-2 border-ink rounded-2xl bg-white p-3.5 flex flex-col min-h-0 h-full overflow-hidden shadow-brutal-sm">
                  ...
                  <div className="flex-1 overflow-y-auto custom-scrollbar space-y-2 pt-2 pr-1">
                    {isDetailLoading ? (
                      <div className="flex items-center justify-center py-12 text-ink text-xs font-bold">
                        <Loader2 size={16} className="animate-spin mr-2" />
                        {t('admin.loading_history')}
                      </div>
                      ...
                      <div className="space-y-2">
                        {userDetailData.activities.content.map((act) => (
                          <div key={act.id} className="bg-canvas border-2 border-ink rounded-xl p-3 flex items-start justify-between text-xs hover:bg-white transition">
              ...
              <div className="flex items-center justify-between pt-3 border-t-2 border-ink shrink-0">
                <div className="flex items-center space-x-2">
                  <button
                    ...
                    className="px-3 py-1.5 rounded-xl border-2 border-ink bg-white text-ink text-xs font-black uppercase hover:bg-ink hover:text-canvas disabled:opacity-40 cursor-pointer flex items-center gap-1 shadow-brutal-sm"
                  >
                    <ChevronLeft size={14} />
                ...
                <button
                  onClick={() => setShowDetailModal(false)}
                  className="px-6 py-2 rounded-xl text-xs font-black uppercase text-canvas bg-ink border-2 border-ink hover:bg-[#2A2A2A] cursor-pointer transition shadow-brutal-sm"
                >
                  {t('admin.close')}
                </button>
              </div>
