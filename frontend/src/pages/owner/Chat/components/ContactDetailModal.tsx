...
export const ContactDetailModal: React.FC<ContactDetailModalProps> = ({
  ...
  return (
    <div
      ...
      className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/50 select-none animate-fade-in cursor-pointer font-['JetBrains_Mono',monospace]"
    >
      <div
        onClick={(e) => e.stopPropagation()}
        className="bg-canvas rounded-3xl border-2 border-ink w-full max-w-4xl overflow-hidden flex flex-col md:flex-row max-h-[90vh] cursor-default"
      >
        <div className="w-full md:w-[280px] border-r-2 border-ink flex flex-col bg-canvas shrink-0">
          <div className="p-6 flex flex-col items-center gap-4 border-b-2 border-ink">
            <ContactAvatar photoUrl={selectedContact.photoUrl} name={selectedContact.firstName} size="lg" />
            <div className="text-center">
              <h2 className="font-['Anybody',sans-serif] text-lg font-black text-ink uppercase leading-tight">
              ...
              {selectedContact.username && (
                <a
                  ...
                  className="inline-flex items-center gap-1 text-xs font-bold text-ink/60 hover:text-ink transition-all mt-0.5"
                >
          ...
          <div className="px-5 py-4 flex flex-col gap-3 border-b-2 border-ink">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-[10px] font-black uppercase px-2 py-0.5 border-2 border-ink rounded-lg bg-white hover:bg-ink hover:text-canvas transition-all cursor-pointer"
              >
                {isUnsubscribed ? t('crm.contact.subscribe') : t('crm.contact.unsubscribe')}
              </button>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-[10px] font-black bg-ink text-canvas px-1.5 py-0.5 rounded font-mono">ID</span>
              <span className="text-xs font-bold text-ink">{selectedContact.telegramId}</span>
            </div>
          </div>
          <div className="p-4 flex flex-col gap-2 mt-auto">
            <button
              onClick={() => handleUpdateContactMetadata({ ...meta, paused: !isPaused })}
              className={`w-full py-2.5 border-2 border-ink rounded-xl text-xs font-black uppercase transition-all flex items-center justify-center gap-2 cursor-pointer ${
                isPaused ? 'bg-emerald-400 text-ink hover:bg-emerald-500' : 'bg-white text-ink hover:bg-ink hover:text-canvas'
              }`}
            >
            ...
            <button
              ...
              className="w-full py-2.5 bg-ink hover:bg-[#2A2A2A] text-canvas border-2 border-ink rounded-xl text-xs font-black uppercase transition-all flex items-center justify-center gap-2 cursor-pointer"
            >
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          <div className="px-6 py-4 border-b-2 border-ink flex items-center justify-between shrink-0">
            <span className="font-['Anybody',sans-serif] text-sm font-black text-ink uppercase tracking-tight">
              {t('crm.contact.details') || 'ДЕТАЛІ'}
            </span>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setShowDeleteConfirm(true)}
                className="w-8 h-8 flex items-center justify-center rounded-xl border-2 border-ink bg-white text-rose-600 hover:bg-rose-500 hover:text-white hover:border-rose-500 transition-all cursor-pointer"
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
            <section className="space-y-3">
              <div className="flex items-center justify-between">
                <h3 className="font-['Anybody',sans-serif] text-xs font-black text-ink uppercase tracking-wider">
                  {t('crm.contact.tags_title')}
                </h3>
                <button
                  onClick={() => setShowAddTagInline(!showAddTagInline)}
                  className="flex items-center gap-1 text-[10px] font-black uppercase px-2.5 py-1 border-2 border-ink rounded-lg bg-white hover:bg-ink hover:text-canvas transition-all cursor-pointer"
                >
                  <Plus size={10} />
                  ...
                  {newTagVal === 'NEW_TAG' ? (
                    <>
                      <input
                        ...
                        className="flex-1 px-3 py-1.5 bg-white border-2 border-ink rounded-xl text-xs font-bold focus:outline-none"
                        autoFocus
                      />
                      <button
                        onClick={() => { handleAddTagInline(customTagName); setCustomTagName(''); }}
                        className="px-3 py-1.5 bg-ink text-canvas border-2 border-ink rounded-xl text-xs font-black uppercase cursor-pointer"
                      >
                        {t('crm.contact.add')}
                      </button>
                      <button
                        onClick={() => { setNewTagVal(''); setCustomTagName(''); }}
                        className="px-3 py-1.5 bg-white text-ink border-2 border-ink rounded-xl text-xs font-black cursor-pointer"
                      >
                        <X size={12} />
                      </button>
                    </>
              ...
              <div className="flex flex-wrap gap-1.5">
                {!(selectedContact.tags) || selectedContact.tags.length === 0 ? (
                  <span className="text-xs text-ink/40 italic font-bold">{t('crm.contact.no_tags')}</span>
                ) : (
                  (selectedContact.tags || []).map((tag) => (
                    <span key={tag} className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-white border-2 border-ink text-ink text-xs font-black uppercase rounded-xl select-none">
                      <span>{tag}</span>
                      <button
                        onClick={() => handleRemoveTagInline(tag)}
                        className="text-ink/50 hover:text-rose-600 cursor-pointer transition-all"
                      >
                        <X size={11} />
                      </button>
                    </span>
            ...
            <section className="space-y-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-ink/50 uppercase tracking-wider block">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white border-2 border-ink rounded-xl text-xs font-bold text-ink focus:outline-none transition-all"
                  />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-black text-ink/50 uppercase tracking-wider block">
                  ...
                  <input
                    ...
                    className="w-full px-3 py-2 bg-white border-2 border-ink rounded-xl text-xs font-bold text-ink placeholder:text-ink/30 focus:outline-none transition-all"
                  />
                </div>
              </div>
            </section>
            ...
            <section className="space-y-3">
              ...
              {showAddCustomFieldInline && (
                <div className="flex flex-col gap-2 bg-white border-2 border-ink p-3 rounded-2xl">
                  {availableFields.length > 0 && (
                    <div className="relative w-full" ref={fieldDropdownRef}>
                      <button
                        type="button"
                        onClick={() => setIsFieldDropdownOpen(!isFieldDropdownOpen)}
                        className="w-full px-3 py-1.5 bg-canvas border-2 border-ink rounded-xl text-xs font-bold text-ink flex items-center justify-between cursor-pointer focus:outline-none select-none"
                      >
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    <button
                      ...
                      className="w-8 h-8 flex items-center justify-center bg-ink text-canvas border-2 border-ink rounded-xl cursor-pointer hover:bg-[#2A2A2A] disabled:opacity-40 disabled:cursor-not-allowed transition-all shrink-0"
                    >
                      <Check size={13} />
                    </button>
                  </div>
              ...
              <div className="space-y-2">
                {!(meta.customFields) || Object.keys(meta.customFields).length === 0 ? (
                  <span className="text-xs text-ink/40 italic font-bold">{t('crm.contact.no_custom_fields')}</span>
                ) : (
                  Object.entries(meta.customFields).map(([k, v]) => (
                    <div key={k} className="flex items-center justify-between py-2 px-3 bg-white border-2 border-ink rounded-xl">
                      <div className="flex flex-col min-w-0">
                        <span className="text-[10px] font-black text-ink/50 uppercase tracking-wider">{k}</span>
                        <span className="text-xs font-bold text-ink truncate">{v}</span>
                      </div>
                      <button
                        onClick={() => handleRemoveCustomFieldInline(k)}
                        className="w-7 h-7 flex items-center justify-center text-ink/40 hover:text-rose-600 hover:bg-rose-50 border-2 border-transparent hover:border-rose-200 rounded-lg transition-all cursor-pointer shrink-0"
                      >
                        <X size={13} />
                      </button>
                    </div>
