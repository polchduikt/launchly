...
export const MessageArea: React.FC<MessageAreaProps> = ({
  ...
  if (!conversation) {
    return (
      <div className="h-full flex flex-col items-center justify-center text-ink gap-3 bg-canvas font-['JetBrains_Mono',monospace]">
        <MessageSquare size={48} strokeWidth={1.5} className="text-ink" />
        <span className="font-['Anybody',sans-serif] text-base font-black uppercase text-ink">{t('crm.chat.select_placeholder_title')}</span>
  ...
  return (
    <>
      <div className="h-[56px] min-h-[56px] px-5 border-b-2 border-ink flex items-center justify-between shrink-0 bg-canvas font-['JetBrains_Mono',monospace]">
        <div className="flex items-center gap-3">
          <UserAvatar name={conversation.botUserName} photoUrl={conversation.botUserPhotoUrl} size={34} />
          <div>
            <h3 className="font-['Anybody',sans-serif] font-black text-sm text-ink uppercase tracking-tight">{conversation.botUserName}</h3>
            {conversation.botUserUsername && (
              <button
                onClick={() => window.open(`https://t.me/${conversation.botUserUsername}`, '_blank')}
                className="text-[10px] text-ink font-bold hover:underline cursor-pointer flex items-center gap-0.5"
              >
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4 bg-canvas font-['JetBrains_Mono',monospace]" style={{ scrollbarWidth: 'none' }}>
        {isMsgLoading ? (
          <MessageAreaSkeleton />
        ) : groupedMessages.length === 0 ? (
          <div className="h-full flex items-center justify-center text-xs text-ink font-bold italic">No messages in this conversation.</div>
        ) : (
          groupedMessages.map((group, gi) => (
            <div key={gi}>
              <div className="flex items-center justify-center my-4">
                <span className="text-[10px] text-ink font-black uppercase bg-white border-2 border-ink px-3 py-1 rounded-full shadow-brutal-sm">
