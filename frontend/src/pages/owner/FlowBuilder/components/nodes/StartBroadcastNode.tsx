...
const StartBroadcastNodeInner: React.FC<NodeProps<Node<CustomNodeData>>> = ({ data = {} }) => {
  ...
  return (
    <div className={`w-64 bg-white/70 backdrop-blur-[2px] border-2 rounded-3xl p-4 shadow-md select-none transition-all border-ink relative overflow-visible isolate ${
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        <span className="font-extrabold text-xs text-ink uppercase tracking-wider">{t('broadcast.builder.node.when')}</span>
      </div>
      <div className="bg-white border border-ink/25 rounded-2xl py-2.5 px-3 text-xs font-bold text-ink/70 text-center">
        {t('broadcast.builder.node.you_send_broadcast')}
      </div>
      <div className="flex justify-end items-center mt-3 pt-2 border-t border-ink/15 relative">
        <span className="text-[9px] font-extrabold text-ink/45 uppercase tracking-wider mr-2">{t('node.start.then')}</span>
        <NodeHandle
          type="source"
