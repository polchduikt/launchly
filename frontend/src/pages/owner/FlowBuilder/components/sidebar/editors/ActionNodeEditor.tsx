    ...
    <div className="space-y-4">
      ...
      <div className="space-y-3 mt-3">
        ...
        <div 
          ...
          className="fixed inset-0 z-[9999] flex items-center justify-center bg-ink/40 p-4 animate-in fade-in duration-200"
        >
          <div 
            onClick={(e) => e.stopPropagation()}
            className="bg-canvas border-2 border-ink rounded-3xl shadow-2xl w-full max-w-[800px] h-[600px] flex overflow-hidden animate-in zoom-in-95 duration-200 relative font-['JetBrains_Mono',monospace] text-ink"
          >
            <button
              onClick={() => {
                setIsActionPickerOpen(false);
              ...
              className="absolute right-4 top-4 p-2 bg-canvas hover:bg-ink hover:text-canvas border-2 border-ink rounded-xl text-ink transition-all cursor-pointer z-50"
            >
              <X size={16} />
            </button>

            {/* Left Sidebar Categories */}
            <div className="w-[210px] border-r-2 border-ink bg-canvas p-5 flex flex-col gap-1.5 select-none shrink-0">
              <h2 className="text-xs font-black text-ink uppercase tracking-wider mb-3 mt-4 leading-snug font-['Anybody',sans-serif]">
            ...
            <div className="flex-1 p-6 overflow-y-auto custom-scrollbar flex flex-col gap-5">
              <div className="flex flex-col gap-3 mr-12">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-9 pr-4 py-2 rounded-xl border-2 border-ink focus:outline-none text-xs font-bold bg-white text-ink placeholder:text-ink/40"
                  />
                </div>
              </div>
              ...
              {searchQuery ? (
                <div className="space-y-4">
                  <div>
                    <h3 className="text-xs font-black text-ink uppercase tracking-wider font-['Anybody',sans-serif]">{t('editor.action.search_results_title')}</h3>
                    <p className="text-[10px] text-ink/70 font-bold mt-0.5">{t('editor.action.search_results_desc', { count: filteredActions.length })}</p>
                  </div>
                  <div className="grid grid-cols-1 gap-3">
                    {filteredActions.map((act) => (
                      <button
                        ...
                        className="w-full text-left p-4 border-2 border-ink hover:bg-ink hover:text-canvas rounded-2xl transition-all cursor-pointer flex gap-3.5 items-center group bg-white text-ink"
                      >
                        <div className="w-9 h-9 rounded-xl bg-amber-100 border-2 border-ink flex items-center justify-center shrink-0 text-ink">
                          {act.icon}
                        </div>
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2">
                            <h4 className="text-xs font-black group-hover:text-canvas transition-colors font-['Anybody',sans-serif]">{act.title}</h4>
                            {act.pro && (
                              <span className="text-[8px] font-black bg-amber-300 text-ink border border-ink px-1.5 py-0.5 rounded uppercase tracking-wider">
                                PRO
                              </span>
                            )}
                          </div>
                          <p className="text-[10px] text-ink/70 group-hover:text-canvas/80 font-bold mt-0.5">{act.desc}</p>
                        </div>
                      </button>
                    ))}
                  </div>
                ...
                <>
                  {selectedCategory === 'recently_used' && (
                    <div className="space-y-3.5">
                      <div>
                        <h3 className="text-xs font-black text-ink uppercase tracking-wider font-['Anybody',sans-serif]">{t('editor.action.categories.recently_used')}</h3>
                        <p className="text-[10px] text-ink/70 font-bold mt-0.5">{t('editor.action.recently_used_desc')}</p>
                      </div>
