      ...
      >
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h3 className="text-sm font-extrabold text-slate-800 uppercase tracking-wide">
            {t('crm.tags.create_tag_title', 'Create tag')}
          </h3>
          <button
            onClick={handleClose}
        ...
        <p className="text-xs text-slate-500 font-medium leading-relaxed">
          {t('crm.tags.modal_desc', 'A tag is simply a label used to describe an identifying characteristic about a contact so you can sort and organize your audience. Tags allow you to segment your contacts.')}
        </p>

        <div className="space-y-3.5">
          <div>
            <label className="block text-[10px] font-extrabold text-slate-500 uppercase tracking-wider mb-1.5 select-none">
              {t('crm.tags.name_label', 'Name')}
            </label>
            <input
              type="text"
              required
              ...
              placeholder={t('crm.tags.enter_tag_name', 'Enter tag name')}
          ...
          <div>
            <label className="block text-[10px] font-extrabold text-slate-500 uppercase tracking-wider mb-1.5 select-none">
              {t('modal.create_field.folder', 'Folder')}
            </label>
            <input
              type="text"
              value={newTagFolder}
              onChange={(e) => setNewTagFolder(e.target.value)}
              placeholder={t('crm.tags.title', 'Tags')}
              className="w-full px-4 py-2.5 rounded-xl border border-slate-200 focus:outline-none focus:border-indigo-500 text-xs font-semibold bg-slate-50/20"
            />
          </div>
        </div>
        ...
        <div className="flex gap-2.5 justify-between pt-2 border-t border-slate-100 mt-2">
          ...
          >
            {isPending ? (t('common.creating', 'Creating...')) : (t('modal.create_field.create', 'Create'))}
          </button>
        </div>
      </div>
    </div>,
