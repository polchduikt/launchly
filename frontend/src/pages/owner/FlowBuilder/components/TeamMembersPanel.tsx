  ...
  return (
    <div ref={dropdownRef} className="relative w-full">
      <button
        ...
        className="w-full px-3.5 py-2 bg-white text-ink text-xs font-bold rounded-xl border-2 border-ink shadow-brutal-sm flex items-center justify-between transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
      >
      ...
      {isOpen && !disabled && (
        <div className="absolute top-full left-0 mt-1.5 w-full bg-canvas border-2 border-ink shadow-brutal rounded-2xl z-50 py-1.5 overflow-hidden animate-in fade-in duration-150">
          {roles.map((role) => {
  ...
  const renderMemberAvatar = (m: TeamMemberResponse, sizeClass = 'w-8 h-8') => {
    ...
    return (
      <SafeAvatar
        src={avatarUrl}
        name={name}
        className={`${sizeClass} rounded-full object-cover border-2 border-ink shrink-0`}
        fallbackClassName={`${sizeClass} rounded-full bg-ink text-canvas font-black flex items-center justify-center text-xs shrink-0 select-none border-2 border-ink`}
      />
    );
  };
      ...
      <div className="space-y-6 text-left font-['JetBrains_Mono',monospace]">
        <div className="flex items-center gap-1.5 text-xs font-black text-slate-600 select-none">
          <button
            ...
            className="hover:text-ink transition-colors cursor-pointer uppercase"
          >
            {t('settings.members.breadcrumb')}
          </button>
          <span>&gt;</span>
          <span className="text-ink uppercase">{getMemberName(editingMember)}</span>
        </div>
        ...
        <div className="bg-canvas border-2 border-ink rounded-2xl divide-y-2 divide-ink/15 overflow-hidden">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 items-center">
            <div className="lg:col-span-3">
              <span className="font-['Anybody',sans-serif] text-xs font-black text-ink uppercase">{t('settings.members.name')}</span>
            </div>
            <div className="lg:col-span-9 flex items-center gap-3">
              {renderMemberAvatar(editingMember)}
              <span className="text-xs font-bold text-ink">{getMemberName(editingMember)}</span>
              {editingMember.isPending && (
                <span className="px-2 py-0.5 bg-amber-200 border-2 border-ink text-ink rounded-md font-black text-[10px] uppercase">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 items-start">
            <div className="lg:col-span-3 pt-1">
              <span className="font-['Anybody',sans-serif] text-xs font-black text-ink uppercase">{t('settings.members.role_label')}</span>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 items-start">
            <div className="lg:col-span-3 pt-1">
              <span className="font-['Anybody',sans-serif] text-xs font-black text-ink uppercase">{t('settings.members.seat_type')}</span>
            </div>
            <div className="lg:col-span-5 flex items-center gap-3">
              <label className="flex items-center gap-3 cursor-pointer select-none">
                ...
                />
                <span className="text-xs font-bold text-ink select-none">{t('settings.members.inbox_seat')}</span>
              </label>
            </div>
            <div className="lg:col-span-4">
              <p className="text-xs text-slate-700 font-bold leading-relaxed">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 items-start">
            <div className="lg:col-span-3 pt-1">
              <span className="font-['Anybody',sans-serif] text-xs font-black text-ink uppercase">{t('settings.members.billing')}</span>
            </div>
            <div className="lg:col-span-5 flex items-center gap-3">
              <label className="flex items-center gap-3 cursor-pointer select-none">
                <input
                  ...
                  className="w-4 h-4 accent-ink cursor-pointer"
                />
                <span className="text-xs font-bold text-ink select-none">{t('settings.members.has_permissions')}</span>
              </label>
            </div>
            <div className="lg:col-span-4">
              <p className="text-xs text-slate-700 font-bold leading-relaxed">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 p-6 items-start">
            <div className="lg:col-span-3 pt-1">
              <span className="font-['Anybody',sans-serif] text-xs font-black text-ink uppercase">{t('settings.members.remove')}</span>
            </div>
            <div className="lg:col-span-5">
              <button
                type="button"
                ...
                className="w-full py-2 bg-rose-200 hover:bg-rose-300 border-2 border-ink text-ink text-xs font-black uppercase rounded-xl select-none cursor-pointer transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
    ...
    <div className="space-y-6 font-['JetBrains_Mono',monospace]">
      <div className="space-y-6 text-left animate-in fade-in duration-150">
        <div className="bg-canvas border-2 border-ink rounded-2xl overflow-hidden">
          <div className="p-5 flex justify-between items-center border-b-2 border-ink">
            ...
            <button
              ...
              className="px-4 py-2 bg-ink hover:bg-[#2A2A2A] text-canvas text-xs font-black uppercase rounded-xl border-2 border-ink transition-all flex items-center gap-1.5 cursor-pointer"
            >
              <Plus size={14} />
          ...
          <div className="p-6">
            <div className="bg-white border-2 border-ink rounded-xl overflow-hidden text-xs">
              <table className="w-full text-left">
                <thead className="bg-slate-100 border-b-2 border-ink font-extrabold uppercase text-ink">
                  <tr>
                ...
                <tbody className="divide-y border-ink/10 font-bold">
                  {members.map((m) => (
                    <tr key={m.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4 flex items-center gap-3">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            <span className="font-extrabold text-ink">{getMemberName(m)}</span>
                            {m.userId === currentUser?.id && (
                              <span className="bg-ink text-canvas text-[9px] px-1.5 py-0.5 rounded uppercase font-black">
                                {t('settings.members.badge.me', 'Це я')}
                              </span>
                            )}
                            {m.isPending && (
                              <span className="bg-amber-200 text-ink text-[9px] px-1.5 py-0.5 rounded uppercase font-black border border-ink">
                      ...
                      <td className="px-6 py-4">
                        {m.inboxSeat && <Check size={16} className="text-ink" />}
                      </td>
                      <td className="px-6 py-4">
                        {m.billingPermission && <Check size={16} className="text-ink" />}
                      </td>
                      <td className="px-6 py-4 text-right">
                        <button
                          onClick={() => setEditingMemberId(m.id)}
                          className="text-ink hover:underline font-black uppercase cursor-pointer"
                        >
      ...
      {isInviteOpen && (
        <div 
          ...
          className="fixed inset-0 z-[9999] flex items-center justify-center bg-ink/40 p-4 animate-in fade-in duration-200 cursor-pointer"
        >
          <form 
            onSubmit={handleSendInvite}
            onClick={(e) => e.stopPropagation()}
            className="bg-canvas border-2 border-ink rounded-3xl p-6 shadow-brutal-xl w-full max-w-md flex flex-col gap-4 animate-in zoom-in-95 duration-200 text-left cursor-default"
          >
            <div className="flex items-center justify-between border-b-2 border-ink pb-3">
              <h3 className="font-['Anybody',sans-serif] text-sm font-black text-ink uppercase tracking-wide">
            ...
            {errorMsg && (
              <div className="px-4 py-2.5 bg-rose-200 border-2 border-ink text-ink text-xs font-bold rounded-xl">
                {errorMsg}
              </div>
            )}
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-[10px] font-black text-ink uppercase tracking-wider mb-1.5">
              ...
              <div>
                <label className="block text-[10px] font-black text-ink uppercase tracking-wider mb-2.5">
                  ...
                  {['Admin', 'Editor', 'Inbox Agent', 'Viewer'].map((role) => (
                    <label key={role} className="flex items-center gap-3 select-none cursor-pointer">
                      <input
                        ...
                        className="w-4 h-4 accent-ink cursor-pointer"
                      />
                      <span className="text-xs font-bold text-ink">{role}</span>
                    </label>
                  ))}
                </div>
              </div>

              <div className="flex items-center justify-between py-2 border-t-2 border-b-2 border-ink/15">
                <span className="text-xs font-bold text-ink">{t('settings.members.invite.assign_inbox')}</span>
              ...
              <div className="flex items-center justify-between pb-2 border-b-2 border-ink/15">
                <span className="text-xs font-bold text-ink">{t('settings.members.invite.grant_billing')}</span>
                <label className="flex items-center gap-2 cursor-pointer select-none">
                  <input
                    type="checkbox"
              ...
              <div className="bg-white border-2 border-ink p-4 rounded-2xl text-xs text-slate-700 font-bold leading-relaxed">
                {getRoleDescription(inviteRole)}
              </div>
            </div>

            <div className="flex justify-end gap-2.5 pt-2 border-t-2 border-ink/15 select-none">
              <button
                type="button"
                onClick={() => setIsInviteOpen(false)}
                className="px-4 py-2.5 bg-white hover:bg-ink hover:text-canvas text-ink text-xs font-black uppercase rounded-xl border-2 border-ink transition-all cursor-pointer"
              >
              ...
              <button
                ...
                className="px-5 py-2.5 bg-ink hover:bg-[#2A2A2A] disabled:opacity-55 text-canvas text-xs font-black uppercase rounded-xl border-2 border-ink transition-all cursor-pointer"
              >
