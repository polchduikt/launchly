...
const AiPage: React.FC = () => {
  ...
  return (
    <DashboardLayout>
      <div className="h-full flex bg-canvas font-['Geist',sans-serif] overflow-hidden">

        {/* Left Sidebar (ChatGPT-style) */}
        <aside className="w-64 sm:w-72 md:w-80 shrink-0 bg-canvas border-r-2 border-ink flex flex-col justify-between h-full font-['JetBrains_Mono',monospace] select-none z-10">
          
          {/* Top: LAUNCHLY AI title aligned with right header */}
          <div className="h-16 border-b-2 border-ink px-6 flex items-center shrink-0">
            <h1 className="font-['Anybody',sans-serif] text-xl font-black text-ink uppercase tracking-tight select-none">
              LAUNCHLY AI
            </h1>
          </div>
          ...
          <div className="p-3 pb-1 shrink-0">
            <button
              ...
              className="w-full bg-white hover:bg-ink hover:text-canvas text-ink border-2 border-ink rounded-xl px-4 py-2.5 flex items-center justify-center text-xs font-black uppercase tracking-wider transition-all cursor-pointer shadow-brutal-sm hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5 disabled:opacity-50"
            >
          ...
          <div className="flex-1 overflow-y-auto p-3 pt-2 space-y-1 custom-scrollbar">
            <div className="px-2 py-1.5 text-[10px] font-black text-ink/60 uppercase tracking-widest">
            ...
            {isSessionsLoading ? (
              ...
              <div className="px-2 py-4 text-xs font-bold text-ink/50 text-center">
                  ...
                  <div
                    ...
                    className={`group flex items-center justify-between px-3 py-2.5 rounded-xl border-2 transition-all cursor-pointer text-xs font-bold ${
                      isActive
                        ...
                        : 'bg-white/60 hover:bg-white text-ink border-transparent hover:border-ink'
                    }`}
                  >
          ...
          <div className="h-[190px] p-4 sm:p-5 border-t-2 border-ink bg-canvas flex flex-col justify-between shrink-0">
            <div className="flex items-center justify-between">
              <span className="font-['Anybody',sans-serif] text-xs font-black text-ink uppercase tracking-tight">
                {t('ai.tokens_used', 'Використання токенів')}
              </span>
              {isUsageLoading && <Loader2 size={13} className="animate-spin text-ink" />}
            </div>
            ...
            <div className="flex items-center gap-4 my-auto">
              <div className="relative w-16 h-16 sm:w-20 sm:h-20 shrink-0 flex items-center justify-center">
                <svg className="w-16 h-16 sm:w-20 sm:h-20 transform -rotate-90" viewBox="0 0 36 36">
                  ...
                  />
                  <path
                    className="text-ink transition-all duration-700 ease-out"
              ...
              <div className="flex-1 min-w-0 space-y-1">
                <div className="text-xs sm:text-sm font-black text-ink font-mono leading-tight truncate">
                  {usage ? `${usage.tokensUsed?.toLocaleString() ?? 0} / ${usage.tokenLimit?.toLocaleString() ?? 0}` : '0 / 0'}
                </div>
                <div className="text-[10px] sm:text-xs font-black text-ink/70 uppercase tracking-wide">
                  {t('ai.tokens', 'токенів')}
                </div>
                <div className="text-[9px] sm:text-[10px] font-bold text-ink/50 leading-tight">
        ...
        {/* Main Chat Area */}
        <div className="flex-1 flex flex-col h-full overflow-hidden min-w-0 bg-canvas">
          
          {/* Top Header - Aligned in height and border with the sidebar */}
          <header className="h-16 border-b-2 border-ink px-6 flex items-center justify-between shrink-0 z-10 font-['JetBrains_Mono',monospace]">
            <div className="flex items-center gap-2">
              <span className="text-xs font-black uppercase text-ink tracking-wider truncate max-w-md">
            ...
            {messages.length === 0 ? (
              <div className="flex-1 flex flex-col items-center justify-center text-center p-6 space-y-6 max-w-2xl mx-auto">
                <div className="space-y-2 select-none">
                  <h2 className="font-['Anybody',sans-serif] text-2xl font-black text-ink uppercase tracking-tight">
                    LAUNCHLY AI
                  </h2>
                  <p className="text-xs font-bold text-ink/70 max-w-md mx-auto">
                    {t('ai.drawer.chat_placeholder', 'Запитайте щось у Launchly AI...')}
                  </p>
                </div>

                <div className="w-full space-y-3 select-none">
                  <p className="text-[10px] font-black text-ink/60 uppercase tracking-widest text-center">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 w-full">
                    {QUICK_QUESTIONS.map((q) => (
                      <button
                        ...
                        className="p-3 bg-white hover:bg-ink hover:text-canvas border-2 border-ink rounded-xl text-xs font-bold text-ink transition-all cursor-pointer shadow-brutal-sm hover:shadow-none hover:translate-x-0.5 hover:translate-y-0.5 text-left"
                      >
                        {t(q)}
                      </button>
                    ))}
                ...
                <div className="space-y-5 overflow-y-auto flex-1 pr-1.5 custom-scrollbar">
                      ...
                      >
                        {isUser ? (
                          <SafeAvatar
                            src={user?.avatar}
                            name={user?.name}
                            className="w-9 h-9 rounded-xl object-cover border-2 border-ink shrink-0"
                            fallbackClassName="w-9 h-9 rounded-xl bg-white text-ink font-bold text-xs flex items-center justify-center border-2 border-ink shrink-0"
                          />
                        ) : (
                          <div className="w-9 h-9 rounded-xl bg-white border-2 border-ink text-ink flex items-center justify-center shrink-0">
                            <AiIcon size={16} />
                          </div>
                        )}

                        <div
                          className={`p-4 rounded-2xl text-xs leading-relaxed border-2 border-ink break-words overflow-hidden ${
                            isUser
                  ...
                  {chatMutation.isPending && (
                    <div className="flex items-start gap-3.5 max-w-[85%] animate-pulse">
                      ...
                      <div className="p-4 rounded-2xl bg-white text-ink border-2 border-ink rounded-tl-none flex items-center gap-2">
          ...
          <footer className="bg-transparent px-4 sm:px-6 pb-6 pt-2 shrink-0 relative font-['JetBrains_Mono',monospace]">
            <div className="max-w-4xl mx-auto space-y-2.5">
              {isLimitReached && (
                <div className="flex items-start gap-2 text-rose-800 bg-rose-200 border-2 border-ink p-2.5 rounded-xl">
              ...
              <div className="flex items-end bg-white border-2 border-ink rounded-2xl transition-all p-2 w-full gap-2 shadow-brutal-sm">
                <textarea
                  ref={textareaRef}
                  rows={1}
                  ...
                  className="flex-1 bg-transparent pl-3 py-2 text-xs font-bold text-ink focus:outline-none transition-all placeholder:text-slate-500 disabled:opacity-55 disabled:cursor-not-allowed resize-none max-h-40 overflow-y-auto leading-relaxed"
                />
                <button
                  disabled={!inputValue.trim() || isLimitReached || chatMutation.isPending}
                  onClick={() => handleSend()}
                  className="w-9 h-9 bg-ink hover:bg-indigo-700 text-canvas rounded-xl border border-ink flex items-center justify-center transition-all cursor-pointer disabled:bg-slate-200 disabled:text-slate-500 disabled:cursor-not-allowed shrink-0 mb-0.5"
                  title="Send message"
                >
