      ...
      <div>
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-12 pt-6 sm:pt-14 pb-12">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 lg:gap-8 items-start">
            ...
            <aside className="lg:col-span-3 bg-canvas border-2 border-ink shadow-brutal sm:shadow-brutal-lg p-2.5 sm:p-4 lg:sticky lg:top-28">
              <div className="font-['JetBrains_Mono',monospace] text-[10px] sm:text-[11px] font-extrabold uppercase tracking-widest text-slate-500 mb-2 sm:mb-3 px-1 sm:px-2 border-b border-ink/20 pb-1.5 sm:pb-2">
                  ...
                  return (
                    <Link
                      ...
                      className={`block px-2.5 py-1.5 sm:px-3.5 sm:py-2.5 border-2 transition-all text-left ${
                        isActive
                          ? 'bg-ink text-canvas border-ink font-black shadow-brutal-sm'
            ...
            <main className="lg:col-span-9 bg-canvas border-2 border-ink shadow-brutal sm:shadow-brutal-xl p-4 sm:p-10 lg:p-12">
              
              <div className="flex items-center justify-between border-b-2 border-ink pb-3 mb-4 sm:mb-8 font-['JetBrains_Mono',monospace] text-[10px] sm:text-xs font-bold tracking-widest text-ink uppercase">
                <span>{t('legal.effective_date', { date: effectiveDate })}</span>
                <span className="bg-ink text-canvas px-2 py-0.5 text-[9px] sm:text-[10px]">{t('legal.badge', 'LAUNCHLY LEGAL')}</span>
              </div>

              <h1 className="font-['Anybody',sans-serif] text-xl sm:text-4xl lg:text-5xl font-black text-ink uppercase tracking-tight mb-4 sm:mb-8 leading-tight">
                {title}
              </h1>

              <div className="space-y-4 sm:space-y-6 font-['Geist',sans-serif] text-xs sm:text-base text-ink leading-relaxed font-medium">
                {children}
              </div>
