  ...
  return (
    <div className="w-full flex flex-col divide-y divide-slate-200">
      {Array.from({ length: 6 }).map((_, i) => (
        <div
          key={i}
          className="w-full px-4 py-3 flex items-start gap-2.5 bg-transparent animate-pulse"
        >
...
export const MessageAreaSkeleton: React.FC = () => {
  return (
    <div className="flex-1 p-6 space-y-5 animate-pulse overflow-hidden bg-white">
      <div className="flex justify-center my-2">
        <div className="w-24 h-5 rounded-full bg-slate-200 border border-slate-200" />
      </div>

      <div className="flex items-end gap-2 max-w-md">
        <div className="w-7 h-7 rounded-full bg-slate-200 shrink-0" />
        <div className="w-56 h-14 rounded-2xl bg-slate-100 border border-slate-200 p-3 space-y-2">
      ...
      <div className="flex items-end justify-end gap-2 max-w-md ml-auto">
        <div className="w-64 h-16 rounded-2xl bg-slate-200 border border-slate-300 p-3 space-y-2">
          <div className="w-full h-2.5 bg-slate-300 rounded" />
      ...
      <div className="flex items-end gap-2 max-w-md">
        <div className="w-7 h-7 rounded-full bg-slate-200 shrink-0" />
        <div className="w-48 h-12 rounded-2xl bg-slate-100 border border-slate-200 p-3 space-y-1.5">
          <div className="w-full h-2.5 bg-slate-300 rounded" />
...
export const TableSkeleton: React.FC<TableSkeletonProps> = ({ rows = 5, columns = 5 }) => {
  return (
    <div className="w-full bg-white border-2 border-[#0A0A0A] rounded-2xl overflow-hidden animate-pulse">
