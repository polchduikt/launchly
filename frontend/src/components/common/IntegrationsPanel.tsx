...
import { SiClaude, SiGooglegemini, SiHubspot } from '@icons-pack/react-simple-icons';
...
export const IntegrationsPanel: React.FC<IntegrationsPanelProps> = ({ botId, onOpenPricing }) => {
  ...
  return (
    <div className="space-y-8 pb-10 font-['JetBrains_Mono',monospace]">
      ...
      <MailchimpCard botId={botId} integration={mailchimpIntegration} onOpenPricing={handleUpgradeClick} />
      ...
      <div className="bg-[#F2EBDD] border-2 border-[#0A0A0A] rounded-3xl p-6 md:p-8 flex flex-col md:flex-row gap-6 md:items-center justify-between transition-all opacity-80 font-['JetBrains_Mono',monospace]">
        ...
        <div className="w-full md:w-1/3 text-xs text-slate-700 leading-relaxed font-bold">
          {t('settings.integrations.hubspot.desc', 'Direct integration with HubSpot CRM contacts and deals is currently under development.')}
        </div>
      </div>
    </div>
  );
